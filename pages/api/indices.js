        ...
        if (customDateQuery) {
            ...
            customDateResults = customDateRows.reduce((acc, row) => {
                if (qodeStrategyIndices.includes(row.indices)) {
                    ...
                    // Only calculate if both dates are valid trading days
                    if (!validTradingDays.has(startDateStr) || !validTradingDays.has(endDateStr)) {
                        acc[row.indices] = '-';
                        return acc;
                    }
                }
            ...
            for (const [index, data] of Object.entries(groupedData)) {
                const customDateData = data.filter(row => {
                    ...
                    return rowDate >= new Date(startDate) && 
                           ...
                           (!qodeStrategyIndices.includes(index) || validTradingDays.has(dateStr));
                });
